...
- `test_grid_model.py` (41) + `test_dpg_headless.py` (9) + `test_inplace_headless.py`
  (7) + `test_hybrid_headless.py` (42) = **99 headless tests** — see "What's verified".
...
- Shift selection: Shift+click range-extend and the geometry drag hit-test
  (`cell_at`) are unit-tested, but the live Shift poll + status-bar hint, the
  mouse-position-to-cell-rect mapping for the drag, and a focused cell not
  hijacking the drag need eyeballing on a real viewport.
...
- **Now built on the hybrid candidate** — the basic-spreadsheet essentials beyond
  ...
  seconds), mouse selection — Shift+click a second cell to extend the selection
  to a rectangle (reliable, no geometry), or hold Shift and drag to draw one;
  while Shift is held the status bar shows a hint, since DearPyGui 2.x exposes no
  settable mouse cursor for a crosshair — with a visible selection rectangle,
  Open/New (`Ctrl+O`/`Ctrl+N`) plus a File menu, and
